...
csv_filename = os.path.join(dirname, '../../datasets/salary/team.csv')
"""
...
"""
Team Salaries Data Frame
"""
df = pd.DataFrame()
"""
1990-1991 시즌 부터 2019-2020 시즌까지 Team Salaries 정보 크롤링
"""
for year in years:
    req = requests.get("https://hoopshype.com/salaries/{0}-{1}/".format(
        year, year + 1))
    html = req.text
    soup = BeautifulSoup(html, 'html.parser')
    """
    Ranking Table > tbody > tr > td[1].a[class="name"] := Team 이름
    Ranking Table > tbody > tr > td[2] := Team Salary 값
    """
...
"""
req = requests.get("https://hoopshype.com/salaries/")
html = req.text
soup = BeautifulSoup(html, 'html.parser')
"""
Ranking Table > tbody > tr > td[1].a[class="name"] := Team 이름
